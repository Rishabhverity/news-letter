    ...
    try:
        with engine.begin() as connection:
            ...
            if result:
                ...
                new_log_id = str(uuid.uuid4())
                insert_log_sql = text("""
                    INSERT INTO log (id, name, email, action, datetime, subscription_id, is_deleted)
                    VALUES (:id, :name, :email, 'unsubscribe', :datetime, :subscription_id, False)
                """)
                connection.execute(insert_log_sql, {
                    'id': new_log_id,
                    'name': new_name if new_name is not None else current_name,
    ...
    try:
        with engine.begin() as connection:
            ...
            if result:
                ...
                # """)
                # connection.execute(soft_delete_logs_query, {"subscription_id": id})
