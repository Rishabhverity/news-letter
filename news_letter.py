...
# Models
class Subscription(db.Model):
    uuid = db.Column(db.String(36), primary_key=True, unique=True, nullable=False, default=generate_uuid)
    name = db.Column(db.String(100), nullable=False)
    email = db.Column(db.String(120), unique=True, nullable=False)
    unsubscribe = db.Column(db.DateTime, nullable=True)
            ...
            subscriptions = [
                {
                    ...
                    'name': row['name'],
                    'email': row['email'],
                    'unsubscribe': row['unsubscribe'].isoformat() if row['unsubscribe'] else None
                }
                for row in result
            ]
        return jsonify({'subscriptions': subscriptions}), 200
    ...
    try:
        with engine.connect() as connection:
            ...
            if result:
                ...
                # Delete the related logs first
                connection.execute(text("DELETE FROM log WHERE subscription_id = :subscription_id"),
                                   {"subscription_id": subscription_uuid})

                # Delete the subscription record
                connection.execute(text("DELETE FROM subscription WHERE uuid = :uuid"),
